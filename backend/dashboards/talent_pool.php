...
// Get job seekers where this organisation showed interest
$stmt = $conn->prepare("
    SELECT 
        u.id, u.name, u.email, u.cv, i.status AS application_status
    FROM users u
    INNER JOIN interests i ON u.id = i.jobseeker_id
...
<?php if ($result->num_rows > 0): ?>
    <?php while($row = $result->fetch_assoc()): ?>
        <div class="jobseeker">
            ...
            <?php if (!empty($row['cv'])): ?>
                <a href="../uploads/<?= htmlspecialchars($row['cv']) ?>" target="_blank">View CV</a><br>
            <?php else: ?>
                No CV uploaded<br>
            <?php endif; ?>
        </div>
